/*判断一个元素是否在数组中*/
export function contains(arr, val) {
  return arr.indexOf(val) != -1 ? true : false;
}

/**
* @param  {arr} 数组
* @param  {fn} 回调函数
* @return {undefined}
*/
export function each(arr, fn) {
  fn = fn || Function;
  var a = [];
  var args = Array.prototype.slice.call(arguments, 1);
  for (var i = 0; i < arr.length; i++) {
    var res = fn.apply(arr, [arr[i], i].concat(args));
    if (res != null) a.push(res);
  }
}

/**
* @param  {arr} 数组
* @param  {fn} 回调函数
* @param  {thisObj} this指向
* @return {Array} 
*/
export function map(arr, fn, thisObj) {
  var scope = thisObj || window;
  var a = [];
  for (var i = 0, j = arr.length; i < j; ++i) {
    var res = fn.call(scope, arr[i], i, this);
    if (res != null) a.push(res);
  }
  return a;
}

/**
* @param  {arr} 数组
* @param  {type} 1：从小到大   2：从大到小   3：随机
* @return {Array}
*/
export function sort(arr, type = 1) {
  return arr.sort((a, b) => {
    switch (type) {
      case 1:
        return a - b;
      case 2:
        return b - a;
      case 3:
        return Math.random() - 0.5;
      default:
        return arr;
    }
  })
}

/**
 * 数组去重合并
 * var m = [1, 2, 2], n = [2,3,3];
 * console.log(combine(m,n));                     // [1, 2, 3] 
 */
function combine() {
  let arr = [].concat.apply([], arguments);  //没有去重复的新数组 
  return Array.from(new Set(arr));
}



/*去重*/
export function unique(arr) {
  if (Array.hasOwnProperty('from')) {
    return Array.from(new Set(arr));
  } else {
    var n = {}, r = [];
    for (var i = 0; i < arr.length; i++) {
      if (!n[arr[i]]) {
        n[arr[i]] = true;
        r.push(arr[i]);
      }
    }
    return r;
  }
  // 注：上面 else 里面的排重并不能区分 2 和 '2'，但能减少用indexOf带来的性能,暂时没找到替代的方法。。。
  /* 正确排重
  if ( Array.hasOwnProperty('from') ) {
      return Array.from(new Set(arr))
  }else{
      var r = [], NaNBol = true
      for(var i=0; i < arr.length; i++) {
          if (arr[i] !== arr[i]) {
              if (NaNBol && r.indexOf(arr[i]) === -1) {
                  r.push(arr[i])
                  NaNBol = false
              }
          }else{
              if(r.indexOf(arr[i]) === -1) r.push(arr[i])
          }
      }
      return r
  }

   */
}

/*求两个集合的并集*/
export function union(a, b) {
  var newArr = a.concat(b);
  return this.unique(newArr);
}

/*求两个集合的交集*/
export function intersect(a, b) {
  var _this = this;
  a = this.unique(a);
  return this.map(a, function (o) {
    return _this.contains(b, o) ? o : null;
  });
}

/*删除其中一个元素*/
export function remove(arr, ele) {
  var index = arr.indexOf(ele);
  if (index > -1) {
    arr.splice(index, 1);
  }
  return arr;
}

/*将类数组转换为数组的方法*/
export function formArray(ary) {
  var arr = [];
  if (Array.isArray(ary)) {
    arr = ary;
  } else {
    arr = Array.prototype.slice.call(ary);
  };
  return arr;
}

/*最大值*/
export function max(arr) {
  return Math.max.apply(null, arr);
}

/*最小值*/
export function min(arr) {
  return Math.min.apply(null, arr);
}

/*求和*/
export function sum(arr) {
  return arr.reduce((pre, cur) => {
    return pre + cur
  })
}

/*平均值*/
export function average(arr) {
  return this.sum(arr) / arr.length
}

/**
 * 判断数组中json是否存在重复key的值
 * @param {*} arr 数组
 * @param {*} key key值
 * @return repeat 返回true false
 */
// let arr = [
//   { userCode: 1, userName: 1 },
//   { userCode: 2, userName: 2 },
//   { userCode: 3, userName: 3 },
//   { userCode: 1, userName: 1 }
// ];
export function arrayJsonRepeat(arr = [], key) {
  // 判断是否有重复值
  let repeat = false;
  for (let i = 0; i < arr.length; i++) {
    for (let j = i + 1; j < arr.length; j++) {
      if (arr[i][key] == arr[j][key]) {
        repeat = true;
        break;
      }
    }
    if (repeat) break;
  }
  return repeat;
}

/**
 * 数组对象去重
 * @param {*} arr 数组
 * @param {*} key 根据key值去重
 * @return {*} Array 返回去重后的数组
 */
// let a = [
//   {value: 1,text: 1},
//   {value: 2,text: 2},
//   {value: 30,text: 3},
//   {value: 1,text: 1}
// ];
// array.arrayJsonRepeatFilter(a,'value');
export function  arrayJsonRepeatFilter(arr,key) {
  let obj = {};
  return arr.reduce((total, item, index, arr)=>{
    obj[item[key]] ? '' : obj[item[key]] = true && total.push(item);
    return total
  },[]);
}

// 数组对象key value 拼接
// let c = [{
//   serverCode: 90217217,
//   serverIp: "10.243.8.243"
// },{
//   serverCode: 90217217,
//   serverIp: "10.243.8.243"
// }]
// 90217217,90217217
export function joinJsonVal(arr,key) {
  let keyJoin = '';
  arr.forEach(item=>{
    return keyJoin =  keyJoin + ',' + item[key]
  });
  return keyJoin.substr(1,keyJoin.length)
}